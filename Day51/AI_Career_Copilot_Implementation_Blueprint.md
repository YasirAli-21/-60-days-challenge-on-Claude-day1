...
**Status:** Day 1 (Requirements) complete. This document is the single source of truth for Days 2–10.
...
### 📝 Step-by-step implementation plan
1. Initialize the project repository (git init, folder structure matching the Day 2 stack decision).
2. Set up the frontend shell: a single page with a placeholder upload area and a "Test Connection" button.
3. Set up the backend function/server with one route, e.g. `/api/analyze`, that currently just echoes a hardcoded test prompt to the Claude API and returns the result.
4. Store the Claude API key as an environment variable / secret — never hardcoded, never sent to the frontend.
5. Wire the frontend "Test Connection" button to call `/api/analyze` and display the raw AI response on screen.
6. Initialize version control and push to a remote repo (GitHub) so Day 9 deployment has something to deploy from.
7. Confirm the local dev server runs and the end-to-end call succeeds.

### 📂 Files and folders to create or modify
- `/frontend/index.html`, `/frontend/style.css`, `/frontend/app.js` (or framework equivalent)
- `/backend/api/analyze.js` (or equivalent per chosen stack)
- `.env` (local only, gitignored) for the API key
- `.gitignore`
- `README.md` (project name, one-line description, "under construction")

### 🔗 APIs, libraries, services, or tools to integrate
- Claude API — confirm current model name/endpoint details at build time rather than assuming
- Chosen PDF/DOCX parsing libraries (installed but not yet used)
